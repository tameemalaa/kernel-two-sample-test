...
Y = all_zeros[indices[100 // 2:]]

Blured_Y = np.array([gaussian_filter(img, sigma=1) for img in Y])  # Apply Gaussian blur

bandwidth = median_heuristic(X, Blured_Y)
...
observed_mmd, p_val, reject, mmd_permutations = permutation_test(
    X, Blured_Y, kernel_func=kernel_func, B=300, alpha=0.05
)
...
plot_mmd_permutation_test(
    ...
    title='MNIST 0 vs 0+blur: MMD Permutation Test',
    filename='results/mnist_0_vs_0blur_mmd.png'
)